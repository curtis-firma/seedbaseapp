...
const LearnMoreModal = ({ open, onOpenChange, onGetStarted }: LearnMoreModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        hideCloseButton
        className={`fixed inset-0 left-0 top-0 translate-x-0 translate-y-0 max-w-none w-full h-full overflow-hidden p-0 rounded-none border-0 bg-background shadow-2xl sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl sm:transition-[width,height,max-width] sm:duration-300 sm:ease-out ${
          showLightPaper 
            ? 'sm:w-[min(900px,calc(100%-2rem))] sm:max-w-[900px] sm:h-[85vh] sm:max-h-[85vh]' 
            : 'sm:w-[min(600px,calc(100%-2rem))] sm:max-w-[600px] sm:h-auto sm:max-h-[85vh]'
        }`}
        style={{

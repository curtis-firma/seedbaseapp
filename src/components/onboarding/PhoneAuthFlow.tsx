...
import { ArrowRight, Check, Sprout, Shield, Copy, Key, Sparkles, Users, Rocket, CheckCircle2, Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';
import { useUser } from '@/contexts/UserContext';
import { normalizePhone, saveUser, makeHexId, getKeyTypeFromRole, truncateHexId, setSessionPhone, isUsernameTaken as isUsernameTakenLocal, formatPhoneDisplay, createDemoUser, type DemoUser } from '@/lib/demoAuth';
import { findUserByPhone as findUserByPhoneDb, getWalletByUserId, getKeyByUserId, createUser as createUserDb, createWallet, createKey, updateUser, isUsernameTaken as isUsernameTakenDb, type DemoUser as DbDemoUser, type DemoWallet, type DemoKey } from '@/lib/supabase/demoApi';
...
type Step = 'welcome' | 'verifying' | 'restoring' // NEW: For returning users
| 'creating-wallet' | 'wallet-reveal' | 'username' | 'role-select' | 'activating-key' | 'key-reveal';
const WALLET_CREATION_STEPS = [{
  label: 'Deriving entropy...',
  duration: 700
}, {
  label: 'Generating keypair...',
  duration: 800
}, {
  label: 'Encrypting secrets...',
  duration: 700
}, {
  label: 'Connecting to Base L2...',
  duration: 600
}];
const KEY_ACTIVATION_STEPS = [{
  label: 'Initializing protocol...',
  duration: 500
}, {
...
const RESTORE_STEPS = [{
  label: 'Restoring your Seed Wallet...',
  duration: 800
}, {
  label: 'Loading keys...',
  duration: 500
...
export function PhoneAuthFlow({
  ...
  useEffect(() => {
    ...
    const runRestore = async () => {
      ...
      const [wallet, key] = await Promise.all([getWalletByUserId(dbUser.id), getKeyByUserId(dbUser.id)]);
  ...
  const currentStepIndex = progressSteps.indexOf(step === 'creating-wallet' ? 'wallet-reveal' : step === 'activating-key' ? 'key-reveal' : step === 'restoring' ? 'verifying' : step);
  return <motion.div initial={{
    ...
    opacity: 0
  }} className={cn("flex flex-col relative overflow-hidden", asModal ? "bg-white rounded-3xl min-h-[500px]" : "fixed inset-0 bg-background z-50")}>
      {/* Animated gradient background */}
      {!asModal && <motion.div className="absolute inset-0 -z-10" style={{
      ...
      {step !== 'creating-wallet' && step !== 'activating-key' && step !== 'verifying' && step !== 'restoring' && <div className={cn("flex justify-center", asModal ? "pt-6 pb-2" : "p-6")}>
          <div className="flex gap-2">
            {progressSteps.map((s, i) => <div key={s} className={cn("w-2 h-2 rounded-full transition-all", i === currentStepIndex ? "w-6 bg-primary" : i < currentStepIndex ? "bg-primary" : "bg-muted")} />)}
          </div>
        </div>}

      <div className={cn("flex-1 flex flex-col items-center justify-center", asModal ? "px-6 py-8" : "px-6 pb-12")}>
        <AnimatePresence mode="wait">
          ...
          y: -20
              ...
              <motion.div className="mx-auto mb-6" initial={{
            scale: 0.8,
            opacity: 0
          }} animate={{
            scale: 1,
            ...
            delay: 0.1
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="bg-white border border-border rounded-xl hover:border-primary/50 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 transition-all shadow-sm">
                  <input type="tel" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value.replace(/\D/g, ''))} placeholder="(555) 123-4567" className="w-full text-center text-2xl font-semibold bg-white py-4 px-4 outline-none placeholder:text-muted-foreground/50 rounded-xl [&:-webkit-autofill]:bg-white [&:-webkit-autofill]:shadow-[0_0_0px_1000px_white_inset] [&:-webkit-autofill:focus]:shadow-[0_0_0px_1000px_white_inset]" autoFocus />
                </div>

                {/* Returning user badge - checking */}
                {isCheckingUser && phoneNumber.length >= 10 && <motion.div initial={{
              opacity: 0,
              y: 10
            }} animate={{
              opacity: 1,
              y: 0
                ...
                {returningUserPreview && !isCheckingUser && <motion.div initial={{
              opacity: 0,
              y: 10
            }} animate={{
              opacity: 1,
              ...
              <button onClick={handleTryDemo} className="text-sm text-muted-foreground hover:text-foreground transition-colors">
                Try demo instead
              </button>
          ...
          {step === 'verifying' && <motion.div key="verifying" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          y: -20
              ...
              repeat: Infinity
                ...
                <motion.div className="absolute w-24 h-24 rounded-full border-2 border-primary" animate={{
              scale: [1, 1.3, 1],
              opacity: [0.5, 0, 0.5]
            }} transition={{
              duration: 1.5,
          ...
          {step === 'restoring' && dbUser && <motion.div key="restoring" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          y: -20
              ...
              <div className="space-y-2 mt-6">
                {RESTORE_STEPS.map((rs, i) => <motion.div key={rs.label} initial={{
              opacity: 0,
              x: -10
            }} animate={{
              ...
              delay: i * 0.15
                    ...
                    {restoreStep > i ? <Check className="h-4 w-4 text-primary" /> : restoreStep === i ? <Loader2 className="h-4 w-4 animate-spin text-primary" /> : <div className="w-4 h-4" />}
                    <span className={cn("text-sm font-mono", restoreStep >= i ? "text-foreground" : "text-muted-foreground")}>
                      {rs.label}
                    </span>
                  </motion.div>)}
              </div>
          ...
          {step === 'creating-wallet' && <motion.div key="creating-wallet" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          y: -20
              ...
              <motion.div className="w-20 h-20 mx-auto mb-8 bg-primary rounded-2xl flex items-center justify-center" animate={{
            rotate: [0, 5, -5, 0]
          }} transition={{
            duration: 2,
            repeat: Infinity
              ...
              <div className="space-y-2 text-left bg-muted/50 rounded-xl p-4 font-mono text-sm">
                {WALLET_CREATION_STEPS.map((ws, i) => <motion.div key={ws.label} initial={{
              opacity: 0,
              x: -10
            }} animate={{
              ...
              delay: i * 0.1
                  ...
                  {creationStep > i ? <Check className="h-3 w-3 text-primary" /> : creationStep === i ? <Loader2 className="h-3 w-3 animate-spin text-primary" /> : <div className="w-3 h-3" />}
          ...
          {/* Wallet Reveal Step */}
          {step === 'wallet-reveal' && pendingUser && <motion.div key="wallet-reveal" initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          y: -20
            ...
            delay: 0.3
              ...
              <motion.button whileTap={{
            scale: 0.98
          }} onClick={() => setStep('username')} className="w-full py-4 bg-primary text-white rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-primary/90 shadow-lg">
                Continue
                <ArrowRight className="h-5 w-5" />
              </motion.button>
            </motion.div>}
          ...
          {/* Username Step */}
          {step === 'username' && <motion.div key="username" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
              ...
              <div className="space-y-4 mb-6">
                <div>
                  ...
                  <div className="relative bg-white border border-border rounded-xl hover:border-primary/50 focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                    ...
                    <input type="text" value={username} onChange={e => setUsername(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, ''))} placeholder="yourname" className="w-full pl-8 pr-4 py-3 bg-transparent rounded-xl outline-none" autoFocus />
                  </div>
                </div>
                
                <div>
                  <label className="text-sm text-muted-foreground mb-1 block text-left">Display Name (optional)</label>
                  <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} placeholder="Your Name" className="w-full px-4 py-3 bg-white border border-border rounded-xl hover:border-primary/50 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all" />
                </div>
              </div>
              
              <motion.button whileTap={{
            scale: 0.98
          }} onClick={handleUsernameSubmit} disabled={username.length < 2} className={cn("w-full py-4 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all", username.length >= 2 ? "bg-primary text-white hover:bg-primary/90 shadow-lg" : "bg-muted text-muted-foreground")}>
                Continue
                <ArrowRight className="h-5 w-5" />
              </motion.button>
            </motion.div>}
          ...
          {/* Role Selection Step */}
          {step === 'role-select' && <motion.div key="role-select" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          y: -20
              ...
              return <motion.button key={option.role} whileTap={{
                scale: 0.98
              }} onClick={() => handleRoleSelect(option.role)} className={cn("w-full p-4 rounded-xl border-2 transition-all text-left", isSelected ? `border-primary ${option.gradient}` : "border-border hover:border-primary/50 bg-card")}>
                      <div className="flex items-center gap-3 mb-2">
                        <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center", isSelected ? "bg-white/20" : option.gradient)}>
              ...
              <motion.button whileTap={{
            scale: 0.98
          }} onClick={handleActivateKey} disabled={!selectedRole} className={cn("w-full py-4 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all", selectedRole ? "bg-primary text-white hover:bg-primary/90 shadow-lg" : "bg-muted text-muted-foreground")}>
          ...
          {/* Activating Key Step */}
          {step === 'activating-key' && selectedRole && <motion.div key="activating-key" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          y: -20
              ...
              <motion.div className={cn("w-20 h-20 mx-auto mb-8 rounded-2xl flex items-center justify-center", ROLE_OPTIONS.find(r => r.role === selectedRole)?.gradient)} animate={{
            rotate: [0, 5, -5, 0]
          }} transition={{
            duration: 2,
            repeat: Infinity
              ...
              {/* Progress bar */}
              <div className="w-full h-2 bg-muted rounded-full overflow-hidden mb-6">
                <motion.div className={cn(ROLE_OPTIONS.find(r => r.role === selectedRole)?.gradient, "h-full")} initial={{
              width: 0
            }} animate={{
              width: `${creationProgress}%`
            }} />
              ...
              {/* Console-style status lines */}
              <div className="space-y-2 text-left bg-muted/50 rounded-xl p-4 font-mono text-sm">
                {KEY_ACTIVATION_STEPS.map((ks, i) => <motion.div key={ks.label} initial={{
              opacity: 0,
              x: -10
            }} animate={{
              opacity: creationStep >= i ? 1 : 0.3,
          ...
          {/* Key Reveal Step */}
          {step === 'key-reveal' && pendingUser && selectedRole && <motion.div key="key-reveal" initial={{
          opacity: 0,
          scale: 0.95
        }} animate={{
          opacity: 1,
          ...
          y: -20
            ...
            type: 'spring',
            delay: 0.2
          }} className={cn("w-20 h-20 mx-auto mb-6 rounded-2xl flex items-center justify-center", ROLE_OPTIONS.find(r => r.role === selectedRole)?.gradient)}>
            ...
            delay: 0.3
                ...
                <div className="flex items-center justify-center gap-2">
                  <TypewriterText text={pendingUser.keyDisplayId || makeHexId(16)} className="font-mono text-lg" />
              ...
              <motion.button whileTap={{
            scale: 0.98
          }} onClick={handleComplete} className="w-full py-4 bg-primary text-white rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-primary/90 shadow-lg">
                Enter Seedbase
...
function TypewriterText({
  ...
  return <span className={className}>
      {displayText}
      {displayText.length < text.length && <motion.span animate={{
      opacity: [1, 0]
    }} transition={{
      duration: 0.5,
      repeat: Infinity
